	...
	<div class="span6" id="divquest">
		<form method="POST" action="index.php?controller=etudiant&amp;action=test">
			...
			<input type="hidden"  name="idGroupe"  value="<?php echo "$idGroupe"; ?>">	
				...
				<input type="submit" class="btn btn-lg btn-dark" name="Precedent" value="Precedent" <?php if($idGroupe == 1){ echo "disabled";} ?>/>
				<?php if($idGroupe != 12){ ?>
				<input type="submit" class="btn btn-lg btn-dark" name="Suivant" value="Suivant" > 
				<?php } else { ?> <input type="submit" class="btn btn-lg btn-dark" name="Terminer" value="Terminer"/> <?php } ?>
		</form>
	</div>
	</center>
</div>
