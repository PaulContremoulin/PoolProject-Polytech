	...
	<div class="span6">
		...
		<form method="POST" action="index.php?controller=etudiant&amp;action=creation">
			<fieldset>
		    	...
		    	<input type="text" placeholder="1234567890A" name="ineEtudiant" pattern="^[0-9]{10}[a-z]$" id="ineEtudiant"  required/>
		    	<label for="mailEtudiant"><h4> E-mail : </h4></label>
		    	<input type="text" placeholder="prenom.nom@etu.umontpellier.fr" name="mailEtudiant" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$" id="mailEtudiant" required/>
		    	...
		    	<input type="password" name="pwdEtudiant" pattern="[0-9a-zA-Z]{6,}" id="pwdEtudiant" required/>
				<label for="confirmPwd"><h4> Confirmez le mot de passe : </h4></label>
		    	<input type="password" name="confirmPwd" pattern="[0-9a-zA-Z]{6,}" id="confirmPwd"  required/>
